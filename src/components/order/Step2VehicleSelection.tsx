    ...
    <div className="max-w-4xl mx-auto">
      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-sm border border-gray-100 p-4 sm:p-6 lg:p-8">
        ...
        <div className="space-y-6 sm:space-y-8">
          ...
          <div className="bg-gradient-to-r from-cyan-50 to-blue-50 rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-cyan-200">
            <div className="flex items-center justify-center space-x-2 text-cyan-700">
              <span className="text-base sm:text-lg">❄️</span>
              <span className="text-xs sm:text-sm font-medium">La opción Refrigerado está disponible para todos los tipos de servicio</span>
            </div>
          </div>
              ...
              >
                ...
                {vehicleType.isRefrigerated && (
                  <div className="absolute -top-1 sm:-top-2 left-2 sm:left-4">
                    <span className="bg-gray-800 text-white text-xs px-2 sm:px-3 py-1 rounded-full font-medium shadow-sm">
                      Refrigerado
                    </span>
                  </div>
                )}
                ...
                <div className="mb-3 sm:mb-4 lg:mb-6 transition-all duration-300 group-hover:scale-105">
                  {vehicleType.isRefrigerated ? (
                    <div className="w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 bg-gradient-to-br from-gray-100 to-gray-200 rounded-lg sm:rounded-[10px] shadow-lg flex items-center justify-center">
                      <vehicleType.icon className="w-10 h-10 sm:w-12 sm:h-12 lg:w-16 lg:h-16 text-gray-600" />
                    </div>
                  ) : (
                    <img
                      src={vehicleType.image}
                      alt={vehicleType.name}
                      className="w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 object-cover rounded-lg sm:rounded-[10px] shadow-lg"
                    />
                  )}
                </div>
                
                {/* Título */}
                <h3 className={`font-semibold text-sm sm:text-base lg:text-lg mb-2 sm:mb-3 transition-colors duration-300 ${
                ...
                {/* Capacidad */}
                <div className={`text-xs sm:text-sm mb-2 sm:mb-4 font-medium transition-colors duration-300 ${
                ...
                <div className="flex flex-wrap gap-1 sm:gap-2">
                  {vehicleType.features.map((feature: string, index: number) => (
                    <span
                      key={index}
                      className={`px-2 sm:px-3 py-1 text-xs rounded-full transition-colors duration-300 ${
                        data.selectedVehicleType?.id === vehicleType.id 
          ...
          {data.selectedVehicleType && (
            <div className="bg-gray-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-200">
              <h3 className="font-medium text-gray-900 mb-2 sm:mb-3 text-sm sm:text-base">Unidad seleccionada</h3>
              <div className="flex items-start space-x-3 sm:space-x-4">
                {data.selectedVehicleType.isRefrigerated ? (
                  <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-gray-100 to-gray-200 rounded-lg sm:rounded-[10px] shadow-md flex items-center justify-center">
                    <data.selectedVehicleType.icon className="w-6 h-6 sm:w-8 sm:h-8 text-gray-600" />
                  </div>
                ) : (
                  <img
                    src={data.selectedVehicleType.image}
                    alt={data.selectedVehicleType.name}
                    className="w-12 h-12 sm:w-16 sm:h-16 object-cover rounded-lg sm:rounded-[10px] shadow-md"
                  />
                )}
                <div className="flex-1">
                  <p className="font-medium text-gray-900 text-sm sm:text-base">{data.selectedVehicleType.name}</p>
                  <p className="text-gray-700 text-xs sm:text-sm mt-1">{data.selectedVehicleType.description}</p>
